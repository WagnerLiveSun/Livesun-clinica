...
ALTER TABLE `users` MODIFY COLUMN `clinicaId` int;--> statement-breakpoint
ALTER TABLE `users` MODIFY COLUMN `role` enum('master','user','admin','recepcao','profissional','cliente') NOT NULL DEFAULT 'user';--> statement-breakpoint
ALTER TABLE `clinic_settings` ADD `email` varchar(320);